                ...
                if event.type == 3:
                    if event.code == 3:
                        if ('roll' not in AUTONOMOUS_INPUT) or not autonomous:
                            ...
                            CMDS['roll'] = 1500+1000*(event.value-127.5)/(255*denom) # RIGHT left / right
                    elif event.code == 4:
                        if ('pitch' not in AUTONOMOUS_INPUT) or not autonomous:
                            # UP / DOWN: code 04 val from 255 (down) to 0 (up)
                            CMDS['pitch'] = 1500+1000*(255-event.value-127.5)/(255*denom) # RIGHT up / down
                    elif event.code == 1:
                        if ('throttle' not in AUTONOMOUS_INPUT) or not autonomous:
                            # UP / DOWN: code 01 val from 255 (down) to 0 (up)
                            CMDS['throttle'] = 1000+1000*((255-event.value)/255)**expo # LEFT up / down
                    elif event.code == 0:
                        if ('yaw' not in AUTONOMOUS_INPUT) or not autonomous:
                            if not headfree:
                                # LEFT / RIGHT: code 00 val from 0 (left) to 255 (right)
                                CMDS['yaw'] = 1500+1000*((event.value-127.5)/255) # LEFT left / right
...
async def send_cmds_to_fc(pipes):
    ...
    while not shutdown:
        ...
        with MSPy(device="/dev/ttyUSB0", loglevel='WARNING', baudrate=115200) as board:
